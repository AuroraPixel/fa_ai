...
export default async function handler(req, res) {
    ...
    try {
        ...
        // 从MinIO删除图片
        const deleteResult = await deleteFile(imageName);
        
        if (deleteResult.success) {
            console.log(`[MinIO] 已删除图片: ${imageName}`);
        } else {
            console.log(`[MinIO] 删除图片出错: ${deleteResult.error}`);
        }
        ...
        if (fs.existsSync(metadataPath)) {
            ...
            console.log(`[元数据] 已从元数据中移除记录: ${imageName}`);
        }
